                            ...
                            <div class="row align-items-center">
                                <div class="col-xl-8 col-xxl-12">
                                    <div class="text-center text-xl-left text-xxl-center px-4 mb-4 mb-xl-0 mb-xxl-4">
                                        ...
                                        @endif
                                        <div>
                                            <form action="/admin/region/{{ $region->id }}" method="POST" enctype="multipart/form-data" style="width: 900px">
                                                @method('put')
                                                @csrf
                                                <div class="mb-3" >
                                                    <label>Nama Desa</label>
                                                    <input class="form-control" name="nama_desa" value="{{ $region->nama_desa }}" required>
                                                </div>
                                                <div class="form-group">
                                                    <label for="exampleFormControlTextarea1" class="form-label">Lokasi Spasial</label>
                                                    <textarea class="form-control" id="exampleFormControlTextarea1" name="geojson" rows="10" required disabled>
                                                        {{ $region->geojson }}
                                                    </textarea>
                                                </div>
                                                <div class="form-group">
                                                    <label>Jumlah Sasaran Tervaksinasi</label>
                                                    <input class="form-control" name="jumlah_sasaran" value="{{ $region->jumlah_sasaran }}" required>
                                                </div>
